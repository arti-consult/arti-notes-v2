...
__turbopack_context__.s({
    ...
    "saveUTMDataToCookie": (()=>saveUTMDataToCookie)
});
...
async function captureUTMParameters(url) {
    ...
    try {
        ...
        if (Object.values(utmParams).some((value)=>value)) {
            const existingData = await getUTMDataFromCookie();
            // Only save new UTM data if we don't already have these parameters stored
            // or if we're in a new session
            if (!existingData || !existingData.utm_source) {
                await saveUTMDataToCookie(utmParams);
            }
        }
        return utmParams;
    } catch (error) {
...
async function captureVisitData() {
    try {
        ...
        const visitData = undefined;
    } catch (error) {
        console.error("Error capturing visit data:", error);
    }
}
async function saveUTMDataToCookie(data) {
    if ("TURBOPACK compile-time truthy", 1) return;
    "TURBOPACK unreachable";
}
...
function TrackingProvider({ children }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        const getOrCreateAnonymousId = async ()=>{
            try {
                // Try to get the cookie from document.cookie first (client-side)
                const cookies = document.cookie.split(";");
                const anonymousIdCookie = cookies.find((cookie)=>cookie.trim().startsWith(`${ANONYMOUS_ID_COOKIE}=`));
                let anonymousId = anonymousIdCookie ? anonymousIdCookie.split("=")[1] : null;
                if (!anonymousId) {
                    ...
                    document.cookie = `${ANONYMOUS_ID_COOKIE}=${anonymousId}; path=/; max-age=${COOKIE_EXPIRY * 24 * 60 * 60}; sameSite=lax${("TURBOPACK compile-time falsy", 0) ? ("TURBOPACK unreachable", undefined) : ""}`;
                }
            } catch (error) {
    ...
    // Capture UTM parameters and visit data when URL changes
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        if (!pathname) return;
        const handleTracking = async ()=>{
            try {
                ...
                const utmParams = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$tracking$2f$utm$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["captureUTMParameters"])(url);
                // Capture visit data if this is a new session
                ...
                // If UTM parameters exist, store them in the database
                if (Object.values(utmParams).some((value)=>value) || pathname === "/") {
                    const cookies = document.cookie.split(";");
                    const anonymousIdCookie = cookies.find((cookie)=>cookie.trim().startsWith(`${ANONYMOUS_ID_COOKIE}=`));
                ...
                console.error("Error in TrackingProvider:", error);
            }
        };
        handleTracking();
    }, [
...
//# sourceMappingURL=%5Broot-of-the-server%5D__efbc097f._.js.map