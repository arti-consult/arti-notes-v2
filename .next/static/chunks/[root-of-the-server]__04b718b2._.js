...
async function captureVisitData() {
    try {
        ...
        const existingData = cookieStore.get(VISIT_COOKIE_NAME);
        if (existingData) return; // Only track first visit
        const visitData = {
            first_visit: new Date().toISOString(),
            referrer: document.referrer || undefined,
            landing_page: window.location.href
        };
        cookieStore.set(VISIT_COOKIE_NAME, JSON.stringify(visitData), COOKIE_OPTIONS);
    } catch (error) {
...
async function saveUTMDataToCookie(data) {
    ...
    try {
        ...
        console.error("Error saving UTM data to cookie:", error);
    }
}
async function getUTMDataFromCookie() {
    ...
    try {
        ...
        console.error("Error getting UTM data from cookie:", error);
        return null;
    }
}
async function getVisitDataFromCookie() {
    ...
    try {
        ...
        console.error("Error getting visit data from cookie:", error);
        return null;
    }
}
            ...
            const getOrCreateAnonymousId = {
                "TrackingProvider.useEffect.getOrCreateAnonymousId": async ()=>{
                    try {
                        ...
                        const anonymousIdCookie = cookies.find({
                            "TrackingProvider.useEffect.getOrCreateAnonymousId.anonymousIdCookie": (cookie)=>cookie.trim().startsWith(`${ANONYMOUS_ID_COOKIE}=`)
                        }["TrackingProvider.useEffect.getOrCreateAnonymousId.anonymousIdCookie"]);
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$compiled$2f$react$2f$index$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["useEffect"])({
        "TrackingProvider.useEffect": ()=>{
            ...
            const handleTracking = {
                "TrackingProvider.useEffect.handleTracking": async ()=>{
                    try {
                        // Get the full URL including query parameters
                        const url = window.location.href;
                        // Capture UTM parameters from URL
                        const utmParams = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$utils$2f$tracking$2f$utm$2e$ts__$5b$app$2d$client$5d$__$28$ecmascript$29$__["captureUTMParameters"])(url);
                        ...
                        if (Object.values(utmParams).some({
                            "TrackingProvider.useEffect.handleTracking": (value)=>value
                        }["TrackingProvider.useEffect.handleTracking"]) || pathname === "/") {
                            const cookies = document.cookie.split(";");
                            const anonymousIdCookie = cookies.find({
                                "TrackingProvider.useEffect.handleTracking.anonymousIdCookie": (cookie)=>cookie.trim().startsWith(`${ANONYMOUS_ID_COOKIE}=`)
                            }["TrackingProvider.useEffect.handleTracking.anonymousIdCookie"]);
            ...
            }["TrackingProvider.useEffect.handleTracking"];
            handleTracking();
        }
    }["TrackingProvider.useEffect"], [
        pathname,
...
//# sourceMappingURL=%5Broot-of-the-server%5D__04b718b2._.js.map