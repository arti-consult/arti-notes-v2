...
function getClientUTMData() {
    try {
        ...
        const utmData = cookies.find((cookie)=>cookie.trim().startsWith(`${UTM_COOKIE_NAME}=`));
...
function getClientVisitData() {
    try {
        const cookies = document.cookie.split(";");
        const visitData = cookies.find((cookie)=>cookie.trim().startsWith(`${VISIT_COOKIE_NAME}=`));
...
function getClientAnonymousId() {
    try {
        const cookies = document.cookie.split(";");
        const anonymousId = cookies.find((cookie)=>cookie.trim().startsWith(`${ANONYMOUS_ID_COOKIE}=`));
...
function setClientUTMData(data) {
    try {
        ...
        document.cookie = `${UTM_COOKIE_NAME}=${cookieValue}; path=/; max-age=${30 * 24 * 60 * 60}; sameSite=lax${("TURBOPACK compile-time falsy", 0) ? ("TURBOPACK unreachable", undefined) : ""}`;
    } catch (error) {
...
function setClientVisitData(data) {
    try {
        const cookieValue = encodeURIComponent(JSON.stringify(data));
        document.cookie = `${VISIT_COOKIE_NAME}=${cookieValue}; path=/; max-age=${30 * 24 * 60 * 60}; sameSite=lax${("TURBOPACK compile-time falsy", 0) ? ("TURBOPACK unreachable", undefined) : ""}`;
    } catch (error) {
...
function setClientAnonymousId(id) {
    try {
        document.cookie = `${ANONYMOUS_ID_COOKIE}=${id}; path=/; max-age=${365 * 24 * 60 * 60}; sameSite=lax${("TURBOPACK compile-time falsy", 0) ? ("TURBOPACK unreachable", undefined) : ""}`;
    } catch (error) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__10be3e3f._.js.map