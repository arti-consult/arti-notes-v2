...
import { createMeetingsFromCalendarEvents, updateMeetingsFromCalendarEvents } from '@/services/calendarEventService';
 ...
 */
export async function syncAllUserCalendars(): Promise<{ success: boolean, syncedUsers: number }> {
  try {
    const supabase = await createClient();
