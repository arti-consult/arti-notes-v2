...
export type SupportedLanguage =
  (typeof transcriptionConfig.language.supported)[number];

// Helper functions
export function isLanguageSupported(lang: string): lang is SupportedLanguage {
  return transcriptionConfig.language.supported.includes(
...
export function validateAudioFile(file: File): boolean {
  return (
    ...
    transcriptionConfig.audio.allowedTypes.includes(
      file.type as (typeof transcriptionConfig.audio.allowedTypes)[number]
    )
  );
}
